...
This repo includes an **optional** Terraform IAM layer (`infra/gcp/cloud_run_api_demo/iam_bindings.tf`) that models a small production team using **Google Groups** (Workspace / Cloud Identity):
...
- `group_prefix` (defaults to `eventpulse` for this repo)
...
Files:
- `infra/gcp/cloud_run_api_demo/log_views.tf` (bucket + sink + view)
- `infra/gcp/cloud_run_api_demo/iam_bindings.tf` (IAM Condition for `viewAccessor`)
