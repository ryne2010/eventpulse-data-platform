...
In `infra/gcp/cloud_run_api_demo/main.tf` we configure cleanup policies with:
...
### Log retention
`infra/gcp/cloud_run_api_demo/log_views.tf` creates a service-scoped log bucket with:
